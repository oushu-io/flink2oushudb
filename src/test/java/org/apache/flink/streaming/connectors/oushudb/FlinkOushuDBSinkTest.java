...
public class FlinkOushuDBSinkTest {

    public static void main(String[] args) throws Exception {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        CheckpointConfig ckpConfig = env.getCheckpointConfig();
        ckpConfig.setCheckpointInterval(10_000);
        ckpConfig.enableExternalizedCheckpoints(CheckpointConfig.ExternalizedCheckpointCleanup.RETAIN_ON_CANCELLATION);
        ...
        DataStream<RowData> out = stream.map((MapFunction<String, RowData>) a -> {
            String[] s = a.split(",");
            return GenericRowData.of(StringData.fromString(s[0]), Integer.valueOf(s[1]));
        });
        ...
        config.setProperty(FlinkOushuDBSink.OUSHUDB_TEMP_PATH, "hdfs://hdfs@aiserver-1:9000/flink/data");
